...
#define margin 10
@interface yuangangViewViewController ()
...
@implementation yuangangViewViewController
{
...
-(void)btnClick{
    if (self.dField.text.length && self.pField.text.length) {
        self.gLabel.text = [NSString stringWithFormat:@"%.4f",0.7851 * [self.dField.text doubleValue] * [self.dField.text doubleValue] * [self.pField.text doubleValue] / 1000];
    }else{
        return;
    }
}
