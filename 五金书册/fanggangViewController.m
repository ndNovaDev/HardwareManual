...
#define margin 10
@interface fanggangViewController ()
...
@implementation fanggangViewController
{
...
-(void)btnClick{
    if (self.dField.text.length && self.pField.text.length) {
        self.gLabel.text = [NSString stringWithFormat:@"%.4f",[self.dField.text doubleValue] * [self.dField.text doubleValue] * [self.pField.text doubleValue] / 1000];
    }else{
        return;
    }
}
